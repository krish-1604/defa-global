...
export default function Navbar({ onContactClick }) {
  ...
  return (
    <>
      {/* Top Navbar */}
      <nav
        className="fixed top-0 z-50 flex items-center justify-between w-full h-16 px-4 py-3 shadow-md md:justify-between sm:px-6 md:px-10 lg:px-16 sm:h-18 md:h-20"
        style={{
          background:
            "linear-gradient(90deg, #001C34 0%, #003664 50%, #001C34 100%)",
        }}
      >
        {/* Logo section - centered on mobile, left on desktop */}
        <div className="flex items-center justify-start flex-1 space-x-2 md:flex-none">
          <Link href="/" className="flex items-center space-x-2">
            <Image src="/images/logo.png" alt="logo" width={36} height={36} />
          </Link>
          <span className="text-[165%] font-vertrio font-bold text-golden mt-1">
            DeFa Global
          </span>
        </div>
          ...
          {navLinks.map((link) =>
            link.label === "Contact" ? (
              ...
              <Link key={link.path} href={link.path}>
                <span
                  className={`${
                    isActive(link.path) ? "text-golden" : "text-white"
                  } font-hk text-[130%] cursor-pointer`}
                >
                  {link.label}
                </span>
              </Link>
        ...
        {!isOpen ? (
          <button
            className="absolute flex items-center md:hidden focus:outline-none right-4"
          ...
          <button
            className="absolute text-white md:hidden right-4"
      ...
      `}
        style={{
          background:
            "linear-gradient(180deg, #001C34 0%, #003664 50%, #001C34 100%)",
        }}
      >
        <nav className="flex flex-col px-6 py-2">
          {navLinks.map((link) => (
            <div
              key={link.label}
              className="text-center border-b border-white/10 last:border-b-0"
            >
              {link.label === "Contact" ? (
                <div className="py-3">
                  <span
